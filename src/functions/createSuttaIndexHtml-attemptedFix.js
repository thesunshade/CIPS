...
export default function createSuttaIndexHtml(indexObject) {
  ...
  function constructLocatorListHtml(locatorListObject) {
    const locatorListHtml = locatorListObject.locators
      ...
      .join("");
    return `<span class="locator-list">${locatorListHtml}</span>`;
  }
  ...
  function constructSublessLocatorList(locatorListObject, subhead) {
    if (subhead === "") {
      const locatorListHtml = locatorListObject.locators
        ...
        .join("");
      return `<div class="sub-word"><span class="locator-list">${locatorListHtml}</span></div>`;
    } else return "";
  }
    ...
    .map(letter => {
      ...
      return `<div class="alphabet-anchor">${letter}</div>
        ${headwordsArray
          .map(headword => {
            ...
            return `<div id="${headwordId}">${constructHeadWordArea(headwordId, headwordWithCounter, headword)}${sortedSubWords
              .map(subhead => {
                ...
                return `
                  ${constructSublessLocatorList(locatorListObject, subhead)}
                  ${subhead === "" && locatorListObject.xrefs.length > 0 ? (sortedSubWords.length === 1 ? '<div class="sub-word">see ' : '<div class="sub-word">see also ') : '<div class="sub-word">' + subhead}
          ...
          .join("")}`; // end of headword loop
    })
    .join(""); // end of alphabet loop
